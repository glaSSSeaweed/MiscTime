...
namespace method{
  ...
  namespace M02{
	using dms = duration<double, std::milli>;
	void GPS2UTC(){
	  gps_time<milliseconds> gt{round<milliseconds>(dms{gps_clock::now().time_since_epoch()})};
...
template<typename T1, typename T2>
void Convert(typename T1::time_point& tp1){
...
int main()
{
  gps_clock::time_point gps( seconds{round<seconds>( gps_clock::now().time_since_epoch()) } );
  std::cout << "GPS -> UTC" << std::endl;
  Convert<gps_clock, utc_clock>(gps);

  utc_clock::time_point utc( seconds{round<seconds>(utc_clock::now().time_since_epoch())});
